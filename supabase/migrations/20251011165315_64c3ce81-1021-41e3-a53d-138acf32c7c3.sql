-- Add database-level validation for JSONB fields
-- This adds validation functions and triggers to ensure data integrity

-- Function to validate hole_pars structure
...
-- Add trigger to matches table for hole_pars validation
DROP TRIGGER IF EXISTS validate_hole_pars_trigger ON public.matches;
CREATE TRIGGER validate_hole_pars_trigger
  BEFORE INSERT OR UPDATE ON public.matches
  FOR EACH ROW
  EXECUTE FUNCTION validate_hole_pars();

-- Function to validate match_scores strokes range
...
-- Add trigger to match_scores table
DROP TRIGGER IF EXISTS validate_match_score_strokes_trigger ON public.match_scores;
CREATE TRIGGER validate_match_score_strokes_trigger
  BEFORE INSERT OR UPDATE ON public.match_scores
  FOR EACH ROW
  EXECUTE FUNCTION validate_match_score_strokes();

-- Function to validate final_scores structure
...
-- Add trigger to match_results table for final_scores validation
DROP TRIGGER IF EXISTS validate_final_scores_trigger ON public.match_results;
CREATE TRIGGER validate_final_scores_trigger
  BEFORE INSERT OR UPDATE ON public.match_results
  FOR EACH ROW
  EXECUTE FUNCTION validate_final_scores();

-- Function to validate forfeited_players structure
...
-- Add trigger to match_results table for forfeited_players validation
DROP TRIGGER IF EXISTS validate_forfeited_players_trigger ON public.match_results;
CREATE TRIGGER validate_forfeited_players_trigger
  BEFORE INSERT OR UPDATE ON public.match_results
  FOR EACH ROW
  EXECUTE FUNCTION validate_forfeited_players();