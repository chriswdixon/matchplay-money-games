-- Fix concurrency control in finalize_match_results function only
...
BEGIN
  -- IMPROVED: Check if already finalized FIRST (no lock needed)
  ...
  -- Now acquire row-level lock to prevent concurrent execution
  SELECT m.status, m.created_by INTO match_status_check, match_creator_id
  FROM public.matches m
  WHERE m.id = p_match_id
  FOR UPDATE;  -- Lock acquired here
  
  -- Double-check after acquiring lock (for safety against race conditions)
  IF EXISTS (SELECT 1 FROM public.match_results WHERE match_id = p_match_id) THEN
