-- Add team_assignments field to match_participants for team-based formats
ALTER TABLE public.match_participants 
ADD COLUMN IF NOT EXISTS team_number INTEGER CHECK (team_number IN (1, 2));

-- Add is_team_format field to matches to indicate if teams are required
ALTER TABLE public.matches
ADD COLUMN IF NOT EXISTS is_team_format BOOLEAN DEFAULT false;

-- Add winners field to match_results to support ties (array of winner IDs)
ALTER TABLE public.match_results
ADD COLUMN IF NOT EXISTS winners UUID[] DEFAULT NULL;

-- Create index for team queries
CREATE INDEX IF NOT EXISTS idx_match_participants_team 
ON public.match_participants(match_id, team_number) 
WHERE team_number IS NOT NULL;

-- Update finalize_match_results function to handle new formats
...
BEGIN
  ...
  SELECT m.status, m.created_by, m.format, m.is_team_format 
  INTO match_status_check, match_creator_id, match_format_check, is_team_match
  ...
  -- Handle team-based formats (Scramble, Best Ball)
  IF is_team_match THEN
  ...
  ELSE
    -- Individual formats (Stroke Play, Match Play)
