...
-- Add a more restrictive policy for INSERT operations (only via system functions)
-- Since we use SECURITY DEFINER functions like finalize_match_results, 
-- those functions can still operate with elevated privileges

-- No direct INSERT/UPDATE/DELETE access for regular users
-- All result management should go through the secure finalize_match_results function
